 ...
 * @author felip
 */
public class ConsultaMedicoView extends javax.swing.JFrame implements MedicoObserver{
    ...
    public ConsultaMedicoView() {
        ...
        medicoController.addViewObserver(this);
        try {
    ...
    private void addAcoes(){
        btCadastrar.addActionListener(e -> {
            CadastroMedicoView cadMedicoView = new CadastroMedicoView(medicoController);
            cadMedicoView.setVisible(true);
        });
        btAlterar.addActionListener(e -> {
            try {
                AlteraMedicoView alteraMedicoView = new AlteraMedicoView(medicoController, getIdMedico());
                alteraMedicoView.setVisible(true);
            } catch (Exception ex) {
        ...
        btVisualizar.addActionListener(e -> {
            try {
                VisualizaMedicoView visualizaMedicoView = new VisualizaMedicoView(medicoController, getIdMedico());
                visualizaMedicoView.setVisible(true);
            } catch (Exception ex) {
                exibirMensagem(ex.getMessage());
            }
        });
        btDeletar.addActionListener(e -> {
            try {
                int opcao = JOptionPane.showConfirmDialog(null, "Deseja mesmo deletar a medico selecionada ?", "Aviso", 0);
                if(opcao == 0){
                    this.medicoController.excluirMedico(getIdMedico());
                }
            } catch (Exception ex) {
                exibirMensagem(ex.getMessage());
            }
